...
v1.2 — May 2026 — Companion to Architecture Spec v1.0
...
**Sources: **E*TRADE (portfolio), Financial Modeling Prep (fundamentals, earnings, ETF data, congressional trades), FRED (macro, volatility, credit, FX), Polygon.io (prices), New York Times (economic news), Alpaca (Phase 2 paper trading), and Anthropic Claude via Azure Foundry (analysis engine).
...
| New York Times API  —  Economic & Editorial News | New York Times API  —  Economic & Editorial News | New York Times API  —  Economic & Editorial News | New York Times API  —  Economic & Editorial News | New York Times API  —  Economic & Editorial News |
| Article Search (keyword filtered) | Article Search API v2 | Free | v1 | Search: Fed, inflation, tariffs, GDP, employment |
| Times Wire (latest articles) | Times Newswire API | Free | v1 | Real-time stream; filter by Business/Economy |
...
| NytApiKey | New York Times API key | Manual / annually | Free: 500 requests/day; Article Search + Wire |
...
**FMP budget detail: **~20 holdings x 1 call each (profile+ratios combined) = 20 calls. Plus 3 ETF look-throughs x 4 calls each = 12 calls. Plus earnings calendar, news, sector averages, economic calendar = ~13 calls. Plus congressional trading (Senate latest + House latest + per-ticker checks) = ~8 calls. Total ~53/day, well within the 250/day free tier limit. NYT API adds ~5 calls/day (500/day limit).
