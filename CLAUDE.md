...
- Runtime knobs `TZ=America/New_York` and `AUTO_EXECUTE_ENABLED=true` are post-deploy app settings (see functionapp.bicep) and are NOT restored by a code deploy — but they ARE wiped if the app is recreated from Bicep. Without `TZ`, crons run in UTC (collector 09:00 UTC = 05:00 ET; auto_executor 09:35 UTC = pre-market → defers forever). Setting any app setting restarts the host — which is also what (re)connects Application Insights if telemetry has gone silent.
