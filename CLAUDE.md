...
- **OverrideHistory**: PK=year-month, RK=`OV-YYYYMMDD-NNN` — one row per override record the analyzer emitted (Phase 4, write-once): `outcome` (accepted/downsized/rejected), `validator_reasons`, `premise_challenged`, `direction`, `magnitude_pp`, `downsized`, `evidence`, `evidence_count`, `falsifier`, `falsifier_date`, `clean_data_only`. Phase-5 outcome-stamp hooks (`outcome_status`/`resolved_correct`) left null until the falsifier matures.
...
**Override protocol** (responsiveness brief Phase 4 — the first phase that **changes report behavior**). The analyzer prompt now **consumes `reference_weights` / `divergences` / `transition_watch`** and executes toward the reference: Section 2 gains a **Reference column** and a **Current-vs-Reference gap**, and **Recommended = Reference ± logged overrides**. Deviating from the reference — *including a "hold" of a sleeve more than `gap_band_pp` off reference* — requires a structured **`overrides[]`** record (OVERRIDE_SCHEMA_V1): `{premise_challenged, direction (de_risk|re_risk), magnitude_pp, evidence[], falsifier, falsifier_date, clean_data_only}`. `analyzer.assert_override_prompt_schema` gates the prompt on load (sentinels `OVERRIDE_SCHEMA_V1`/`reference_weights`/`overrides`), mirroring the flex gate. Records are validated by the pure `shared/overrides.py::validate_overrides` (Tier-2): structural gates (falsifier + date + clean non-empty evidence + within `max_magnitude_pp` band + valid direction) reject; the **de-risk/re-risk asymmetry** (spec §6) accepts a de-risk override on one clean item, **downsizes** (halves magnitude) an under-evidenced re-risk override, and **rejects** one with no evidence. An override can never loosen the deployment gate or breach a Tier-1 limit (floor/ceiling/exempt). Decisions persist write-once to the new **`OverrideHistory`** table (`analyzer._write_override_history`, layer-tagged, PK=year-month) with Phase-5 outcome-stamp hooks left null. Config: `risk-limits.json` → `override_protocol` (`max_magnitude_pp` 15, `re_risk_min_evidence` 2, `gap_band_pp` 5). **Auto-execute stays OFF-gated; human approval unchanged; paper only.**
