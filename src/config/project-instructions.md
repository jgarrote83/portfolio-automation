...
- `leading_growth` — **pre-computed leading-growth composite** (#17, 2026-07-23): `direction` (`rising`/`falling`/`flat`), `score` (diffusion in [−1,+1]), `confidence` (`full`/`medium`/`low`/`none`), `available`, `signals` (per-signal name/direction/as_of). Sources: WEI, NFCI (inverted), PERMIT, NEWORDER, Philly-Fed new orders, Empire-State activity, CPER/GLD 20d ratio, XLY/XLP 20d ratio, HY OAS trend. **Describe-only** — never flips the growth axis directly; the deterministic `growth_axis` governs the regime call. A `leading_vs_lagging_growth` divergence fires when the composite disagrees with realized. Surface one sentence in Section 1 and weigh in Section 2. An `active` divergence is evidence for overrides; an `indeterminate` one is not.
- `market_implied_quadrant` — **market-implied quadrant** from cross-asset tape momentum (#18, 2026-07-23): `implied_quadrant` (Q1-Q4 or `"borderline"`), `confidence` (`high`/`medium`/`low`/`none`), `implied_growth`/`implied_inflation`, `votes` (per-signal table). Works at borderline regimes — **no dependence on `active_quadrant`** — superseding `price_vs_regime`'s blind spot (which goes indeterminate at borderline regimes, exactly when it is most needed). **Describe-only** — never touches `reference_weights`. A `market_vs_macro_quadrant` divergence fires when the tape disagrees. Surface one sentence in Section 1 and weigh in Section 2. The historical rationale: when tape and realized macro disagree at turns, the tape is early more often than wrong (2022 canonical). A high-confidence tape disagreement is a legitimate de-risk override input; a low-confidence one is not.
- `dollar_proxy` — **daily dollar proxy** from FX pairs when `DTWEXBGS` is >5d stale (#18 sub-item): `available`, `proxy_direction` (`stronger`/`weaker`/`flat`), `components` (per-pair EUR/JPY/CNY), `as_of`. Consume instead of the stale DTWEXBGS for the DXY trend when `available: true`. If `available: false` and DTWEXBGS is stale, note the DXY cadence gap.
- `pnl_decomposition` — **FIFO realized + current unrealized P&L by bucket** (Task C, 2026-07-23): buckets `core_current`, `legacy_exits`, `off_roster_flex` — each `{realized_usd, unrealized_usd, total_usd, pct_of_equity, contributors[{symbol, ...}]}`. Answers the inception-shortfall question. **Quote `§1 scoreboard one sentence only:** "Core book +X / Legacy exits +Y / Off-roster +Z (FIFO realized + current unrealized)." Do not recompute these figures from `portfolio.positions`.
...
9. **Recommendations** — prose summary of the trades proposed in Part 2.
   ...
   **Dynamic watch_candidates (Task A4):** You MAY emit an optional top-level
   ...
   accepted). **Only emit names NOT already in `flex_candidates`** — emitting a name already in the static seed (ETN, NEE, XLU, MU) is a no-op (the collector already collects it). Do NOT waste slots on: held names, any core role-pool member (CORE_ROSTER), non-re-enterable legacy exits (AMZN/GOOGL/DBA/TIP/XSD), or malformed symbols — the collector silently drops them. *(Current static seed: ETN, NEE, XLU, MU — names already here need no `watch_candidates` slot.)* Earnings dates auto-populate for names within 14 days once the collector picks up the name; do not waste a slot to request a catalyst that the collector's earnings calendar already covers. Persistence is **last-emission-only**: re-emit a name each run to keep it in the funnel; a name you stop emitting falls out after one run.
